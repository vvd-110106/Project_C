...
void managerMenu() {
    ...
    printf("       [6]. Search User Id \n");
    printf("       [7]. Unlock ( lock ) User\n");
    printf("       [8]. Sort Users By Name \n");
    printf("       [0]. Exit\n");
}
...
int is_unique(char *name, char *phone, char *email, char *username) {
    for (int i = 0; i < userCount; i++) {
        if (strcmp(users[i].username, username) == 0 || strcmp(users[i].phone, phone) == 0 ||
            ...
            return 0;  // Trung lap thong tin
        }
    }
    return 1;  // Khong trung lap
}

// Ham hien thi danh sach nguoi dung ////////////////////////////////////////////////////// DEREEK//////////////////////////////////////////////////////////////////////////
void printUsers() {
    FILE *file = fopen("bankData.dat", "rb");  // Mo file o che do doc nhi phan
    if (file == NULL) {
...
void lockUnlock() {
    int lockUser;
    int found = 0;
    printf("Nhap ID nguoi dung muon khoa (hoac mo khoa): ");
    scanf("%d", &lockUser);
    for (int i = 0; i < userCount; i++) {
        if (users[i].id == lockUser) {
            found = 1; 
            if (strcmp(users[i].status, "locked") == 0) {
                printf("Nguoi dung %s (%d) dang bi khoa. Ban co chac muon mo khoa? (y/n): ", users[i].name, lockUser);
                char choiceLock;
                scanf(" %c", &choiceLock);
                if (choiceLock == 'y' || choiceLock== 'Y') {
                    strcpy(users[i].status, "open"); 
                    printf("Nguoi dung %s (%d) da duoc mo khoa thanh cong!\n", users[i].name, lockUser);
                } else {
                    printf("Yeu cau mo khoa da bi huy.\n");
                }
            } 
			else if (strcmp(users[i].status, "open") == 0) {
                printf("Nguoi dung %s (%d) dang mo. Ban co chac muon khoa? (y/n): ", users[i].name, lockUser);
                char choiceLock;
                scanf(" %c", &choiceLock);
                if (choiceLock == 'y' || choiceLock == 'Y') {
                    strcpy(users[i].status, "locked"); 
                    printf("Nguoi dung %s (%d) da duoc khoa thanh cong!\n", users[i].name, lockUser);
                } else {
                    printf("Yeu cau khoa da bi huy.\n");
                } 
            }
            break; 
        }
    ...
    if (found==0) {
        printf("Khong tim thay nguoi dung co ID: %od\n", lockUser);
    }
}
